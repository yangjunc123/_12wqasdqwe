layui.use(['jquery','layer', 'table','form','laydate'], function() {
    //当有layui和ztree中均使用Jquery，且版本不一致，则在这里最好不要定义layui的Jquery对象，因为其Jquery版本不一致，不能混合使用
    ...
    table.render({  //数据表格的数据渲染(此UI框架底层是进行异步加载)
        ...
        , cols: [[ //表头
            ...
            , {field: 'flag', title: '是否显示', align: 'center', width: 160, sort: true,templet:'#flagTpl'}
    ...
    table.on('tool(test)', function(obj) {
        ...
        if(layEvent==='query'){
            ...
            layer.open({
                ...
                cancel: function(index, layero){  //点击关闭弹框的回调函数
                    //将树型图的div重新隐藏起来
                    $("#ztreeDiv").hide();
                }
            });
    ...
    function loadZtree(dataUrl) {
        var setting = {
            ...
            async : {
                ...
                url:dataUrl,    //服务器端访问路径，由此路径获取到改角色的权限数据
                autoParam:["id", "name=n", "level=lv"],  //使用异步加载的默认配置
                otherParam:{"otherParam":"zTreeAsyncTest"}
            }
        };
        //$("#test1")为树型数据容器   setting为样式和数据的设置
